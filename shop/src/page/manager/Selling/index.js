...
import BanGiaoHang from "../BanGiaoHang/index";
...
function Selling() {
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      const giamVoucher = Number(
        orderSummary.giamVoucher ?? orderSummary.discount ?? 0
      );
      const giamDiem = Number(orderSummary.giamDiem ?? 0);

      const phiCOD =
        paymentMethod === "cash" ? Number(orderSummary.phiCOD ?? 0) : 0;

      const payload = {
        khachHang_id: selectedCustomer?.id ? String(selectedCustomer.id) : "0",
      ...
      // Tạo dữ liệu hóa đơn cho in (giữ nguyên logic cũ)
      const hoaDonData = {
  ...
  const handleSelectProduct = (productId) => {
    ...
    const gia = Number(selected.gia) || 0;

    const productToAdd = {
      id: selected.id,
      code: selected.maSKU,
      name: selected.tenSanPham,
      image: `http://127.0.0.1:8000/storage/${selected.hinhAnh}`,
      quantity: 1,
      price: gia,
      discount: 0,
      discountType: "vnd",
